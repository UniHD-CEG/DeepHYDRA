...
def run_merlin(data: np.ndarray,
    ...
    distances_all = load_numpy_array('distances_smd.npy')
    discords_all = load_numpy_array('discords_smd.npy')
    lengths_all = load_numpy_array('lengths_smd.npy')
    ...
    results_combined.to_csv('results_merlin_combined_smd.csv', sep='\t')
...
def get_preds_best_threshold(data: np.ndarray,
    ...
    save_numpy_array(pred_reduced, 'preds_smd_reduced_merlin.npy')
