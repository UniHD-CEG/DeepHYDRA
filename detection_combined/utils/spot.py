...
class SPOT:
    ...
    def run_online(self, data_point, dynamic=True):
        """
        Run SPOT in an online fashion
        """
        
        if self.fitted != True:
            raise RuntimeError('SPOT instance is not fitted')
        
        self.add(data_point)

        if not dynamic:
            if self.data[-1] > self.init_threshold:
                self.extreme_quantile = self.init_threshold
                return True
            
        else:
            # If the observed value exceeds the current threshold (alarm case)
            if self.data[-1] > self.extreme_quantile:
                # if we want to alarm, we put it in the alarm list
                return True

            # case where the value exceeds the initial threshold but not the alarm ones
            elif self.data[-1] > self.init_threshold:
                # we add it in the peaks
                self.peaks = np.append(self.peaks, self.data[-1] - self.init_threshold)
                self.Nt += 1
                self.n += 1
                # and we update the thresholds
