...
class TranAD(nn.Module):
    def __init__(self, feats):
        ...
        # self.n = self.n_feats * self.n_window
        # self.pos_encoder = PositionalEncoding(2*feats, 0.1, self.n_window)
        # encoder_layers = TransformerEncoderLayer(d_model=2*feats, nhead=feats//2, dim_feedforward=2048, dropout=0.1)
        # self.transformer_encoder = TransformerEncoder(encoder_layers, 3)
        # decoder_layers1 = TransformerDecoderLayer(d_model=2*feats, nhead=feats//2, dim_feedforward=2048, dropout=0.1)
        # self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 2)
        # decoder_layers2 = TransformerDecoderLayer(d_model=2*feats, nhead=feats//2, dim_feedforward=2048, dropout=0.1)
