...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
    ...
    train_data = np.load(os.path.join(prefix, dataset + '_train.npy'))[train_start:train_end, :]

    try:
        test_data = np.load(os.path.join(prefix, dataset + '_test.npy'))[test_start:test_end, :]

    except (KeyError, FileNotFoundError):
        test_data = None

    test_label = np.load(os.path.join(prefix, dataset + '_labels.npy'))

    test_label = np.add.reduce(test_label, axis=1)
    test_label = np.where(test_label >= 1, 1, 0)

    test_label = test_label.astype(np.uint8)

    try:
        labeled_train_data = np.load(os.path.join(prefix, dataset + '_labeled_train.npy'))

    except (KeyError, FileNotFoundError):
        labeled_train_data = None

    labeled_train_label = np.load(os.path.join(prefix, dataset + '_labeled_train_labels.npy'))
    labeled_train_label = np.where(labeled_train_label >= 1, 1, 0)

    labeled_train_label = labeled_train_label.astype(np.uint8)

    return (train_data, None), (test_data, test_label), (labeled_train_data, labeled_train_label)
